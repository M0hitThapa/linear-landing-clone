...
import { IconArrowLeft, IconArrowRight, IconMenu } from "@tabler/icons-react";
...
const product: NavDropdownItem[] = [
  ...
  { title: "Monitor", href: "/", description: "Understand progress at scale" },
  {
    title: "Plan",
    href: "/",
    description: "Plan and navigate from idea to launch",
  },
  { title: "Diffs", href: "/", description: "Make code review effortless" },
  { title: "Integrations", href: "/", description: "Collaborate across tools" },
];

const resources: NavDropdownItem[] = [
  { title: "About", href: "/", description: "Meet the team" },
  { title: "Careers", href: "/", description: "We're hiring" },
  { title: "Developers", href: "/", description: "Build on the Linear API" },
  { title: "Security", href: "/", description: "Safe, secure, and private" },
  { title: "Docs", href: "/", description: "How to use Linear" },
  { title: "Switch to Linear", href: "/", description: "Migration guide" },
  { title: "", href: "", description: "" },
  { title: "Download", href: "/", description: "Get the desktop app" },
  { title: "Mobile", href: "/", description: "Get the mobile app" },
];
...
const dropdownMenus = [
  { label: "Products", items: product, className: "" },
  { label: "Resources", items: resources, className: "hidden md:flex" },
];

const DropdownGrid = ({ items }: { items: NavDropdownItem[] }) => (
  <ul className="grid w-[550px] gap-5 p-2 md:w-[650px] md:grid-cols-3 lg:w-[750px] ">
...
export const Navbar = () => {
  ...
  return (
    <>
      <div
        className="sticky top-0 z-50 w-full h-18 py-4
        bg-linear-to-br
        from-[#08090a]
        via-[#08090a]/90
        to-[#08090a]
        backdrop-blur-2xl
        border-b border-[#1D1E1E] "
      >
        <Container className="flex items-center justify-between ">
          ...
          <NavigationMenu className="flex items-center justify-between gap-5">
            <NavigationMenuList className="hidden md:flex">
              {dropdownMenus.map(({ label, items, className }) => (
                <NavigationMenuItem key={label} className={className}>
                  <NavigationMenuTrigger className="rounded-full  text-neutral-400">
                    {label}
                  </NavigationMenuTrigger>

                  <NavigationMenuContent className=" backdrop-blur-sm bg-[#151617]   ">
                    <DropdownGrid items={items} />
                  </NavigationMenuContent>
                </NavigationMenuItem>
              ))}
            ...
            <div className="h-[15px] w-px bg-neutral-700 hidden md:flex " />
      ...
      {isOpen && (
        <div className="bg-[#08090A]/50 backdrop-blur-2xl  p-5 min-h-screen w-screen  z-50 absolute ">
          Hello
        </div>
      )}
    </>
  ...
  return (
    <li {...props}>
      <NavigationMenuLink asChild>
        <Link href={href}>
          <div className="flex flex-col gap-1.5 text-xs p-2">
            <div className="leading-none font-medium text-neutral-500">
              {title}
            </div>
            <div className="line-clamp-2 text-neutral-200">{children}</div>
          </div>
        </Link>
      </NavigationMenuLink>
    </li>
